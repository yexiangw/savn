...
def loadData():
    data_url = "data/2.0/raw/data.json"
    dataset_url = "https://github.com/budzianowski/multiwoz/raw/master/data/MultiWOZ_2.0.zip"
    if not os.path.exists("data"):
        os.makedirs("data")
    if not os.path.exists("data/2.0"):
        os.makedirs("data/2.0")
        os.makedirs("data/2.0/raw")
    if not os.path.exists("data/multi-woz"):
        os.makedirs("data/multi-woz")
    if not os.path.exists("data/multi-woz"):
        os.makedirs("data/multi-woz")

    if not os.path.exists('data/multi-woz/MultiWOZ_2.0/data.json'):
        print("Downloading and unzipping the MultiWOZ2.0 dataset")
        resp = urllib.request.urlopen(dataset_url)
        zip_ref = ZipFile(BytesIO(resp.read()))
        zip_ref.extractall("data/multi-woz")
        zip_ref.close()
        os.rename("data/multi-woz/MULTIWOZ2 2", "data/multi-woz/MultiWOZ_2.0")

    if not os.path.exists(data_url):
        shutil.copy('data/multi-woz/MultiWOZ_2.0/data.json', 'data/2.0/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.0/valListFile.json', 'data/2.0/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.0/testListFile.json', 'data/2.0/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.0/dialogue_acts.json', 'data/2.0/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.0/ontology.json', 'data/2.0/')
...
def createData():
    ...
    fin1 = open('data/2.0/raw/data.json', 'r')
    data = json.load(fin1)

    fin2 = open('data/2.0/raw/dialogue_acts.json', 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open('data/2.0/raw/testListFile.json', 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open('data/2.0/raw/valListFile.json', 'r')
    for line in fin:
        valListFile.append(line[:-1])
    fin.close()

    trainListFile = open('data/2.0/trainListFile', 'w')
    ...
    # save all dialogues
    with open('data/2.0/dev_dials.json', 'w') as f:
        json.dump(val_dials, f, indent=4)

    with open('data/2.0/test_dials.json', 'w') as f:
        json.dump(test_dials, f, indent=4)

    with open('data/2.0/train_dials.json', 'w') as f:
