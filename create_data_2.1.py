...
def loadData():
    data_url = "data/2.1/raw/data.json"
    dataset_url = "https://github.com/budzianowski/multiwoz/raw/master/data/MultiWOZ_2.1.zip"
    if not os.path.exists("data"):
        os.makedirs("data")
    if not os.path.exists("data/2.1"):
        os.makedirs("data/2.1")
        os.makedirs("data/2.1/raw")
    if not os.path.exists("data/multi-woz"):
        os.makedirs("data/multi-woz")
    if not os.path.exists("data/multi-woz"):
        os.makedirs("data/multi-woz")

    if not os.path.exists('data/multi-woz/MultiWOZ_2.1/data.json'):
        print("Downloading and unzipping the MultiWOZ2.1 dataset")
        resp = urllib.request.urlopen(dataset_url)
        zip_ref = ZipFile(BytesIO(resp.read()))
        zip_ref.extractall("data/multi-woz")
        zip_ref.close()
    if not os.path.exists(data_url):
        shutil.copy('data/multi-woz/MultiWOZ_2.1/data.json', 'data/2.1/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.1/valListFile.txt', 'data/2.1/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.1/testListFile.txt', 'data/2.1/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.1/system_acts.json', 'data/2.1/raw/')
        shutil.copy('data/multi-woz/MultiWOZ_2.1/ontology.json', 'data/2.1/')
...
def createData():
    ...
    fin1 = open('data/2.1/raw/data.json', 'r')
    data = json.load(fin1)

    fin2 = open('data/2.1/raw/system_acts.json', 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open('data/2.1/raw/testListFile.txt', 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open('data/2.1/raw/valListFile.txt', 'r')
    for line in fin:
        valListFile.append(line[:-1])
    fin.close()

    trainListFile = open('data/2.1/trainListFile', 'w')
    ...
    # save all dialogues
    with open('data/2.1/dev_dials.json', 'w') as f:
        json.dump(val_dials, f, indent=4)

    with open('data/2.1/test_dials.json', 'w') as f:
        json.dump(test_dials, f, indent=4)

    with open('data/2.1/train_dials.json', 'w') as f:
